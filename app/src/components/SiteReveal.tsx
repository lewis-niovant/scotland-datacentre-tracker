...
import {
  bboxOf, buildingMasses, centroidOf, pitchGrid, polysOf, primaryBoundary, projectedExtent,
  STOREY_M, type MassSpec,
} from '../lib/geometry'
import { EXTENT2, EXTENT3, NOMINAL_BLOCK_M2, REDLINE } from './MapView'
...
export function hasSiteReveal(geo: GeoCollection, p: ProjectRecord): boolean {
  return !!(primaryBoundary(geo, p.project.slug) ?? projectedExtent(geo, p.project.slug))
}
...
export default function SiteReveal({ geo, p, pitchM2 }: {
  ...
  const extent = boundary ? null : projectedExtent(geo, p.project.slug)
  const shape = boundary ?? extent
  ...
  const areaM2 = boundary
    ...
    : (extent?.properties.area_m2 as number | undefined)
  const tier: 1 | 2 | 3 = boundary ? 1 : extent?.properties.tier === 3 ? 3 : 2
  ...
  const stages: Stage[] = useMemo(() => [
    ...
    {
      key: 'extent',
      label: tier === 1 ? 'Red line' : 'Projected size',
      caption: tier === 1
  ...
  const allStages: Stage[] = useMemo(() => {
    ...
    if (areaM2 && pitchM2) {
      out.push({
        key: 'pitches',
        label: 'Scale',
        caption: `About ${fmtInt(Math.round(areaM2 / pitchM2))} football pitches, drawn at ${fmtInt(pitchM2)} m² each.`,
      })
    }
    if (height != null) {
      out.push({
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      const colour = tier === 1 ? REDLINE : tier === 2 ? EXTENT2 : EXTENT3
      map.addLayer({
      ...
      map.addLayer({
        ...
        paint: {
          ...
          ...(tier !== 1 ? { 'line-dasharray': tier === 2 ? [2.6, 1.7] : [0.6, 1.6] } : {}),
        },
      })
      map.addLayer({
  ...
  useEffect(() => {
    ...
    map.setPaintProperty('sr-fill', 'fill-opacity', on('extent') ? (tier === 3 ? 0.05 : 0.14) : 0)
  ...
  return (
    <section className="section reveal-section">
      ...
      <p className="figure-note">
        Pitches and building blocks are drawn by this site from sourced areas and heights — the
        layout, orientation and parcel shape of anything other than an official red line are not
        evidence.
      </p>
    </section>
  )
